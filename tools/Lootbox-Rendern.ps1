...
#   passiert. Ohne Fenster laeuft derselbe Bau in wenigen Sekunden und
...
# arbeitet Zeile fuer Zeile gleich; wer hier etwas aendert, sieht dort nach.
# ---------------------------------------------------------------------------

$ErrorActionPreference = "Stop"

$hier = Split-Path -Parent $MyInvocation.MyCommand.Path
$bauplan = Join-Path $hier "Lootbox-Blender.py"
if (-not (Test-Path -LiteralPath $bauplan)) {
    Write-Host "Lootbox-Blender.py fehlt neben diesem Skript." -ForegroundColor Red
    exit 1
}
...
$ordner = Join-Path (Split-Path -Parent $hier) "img\lootboxen"
$bilder = Get-ChildItem -LiteralPath $ordner -Filter "lootbox-*.png" -ErrorAction SilentlyContinue
Write-Host ""
